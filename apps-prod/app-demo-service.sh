#!/usr/bin/env bash
# 启动|停止 app-demo-service.jar 文件；  chmod +x app-demo-service.sh
# 注意：在sh文件中=赋值，左右两侧不能有空格

# 应用运行目录
APP_HOME=~/git-repository/common-mvc-parent/apps-prod
# 应用名称(jar包名称)：{APP_NAME}.jar
APP_NAME=app-demo-service
SERVER_PORT="--server.port=3367"
...
START_OPTS="$SERVER_PORT --APP_NAME=$APP_NAME"
echo $APP_HOME
echo $APP_NAME
