...
@Data
@Builder
public class TokenCache implements ICache, ITabUserCache, Serializable {
    private static final String CACHE_TOKEN = "TokenCache";

    /**
     * base64 解码并转换为 {@link TokenCache} 实体，执行 token 认证，认证失败会抛出异常，认证成功则更新 token 过期时间
     *
     * @param base64 {@link String} HTTP 请求头中获取到的 token
     * @return {@link TokenCache}
     */
    public static TokenCache auth(final String base64) {
    ...
    public TokenCache check() {
        ...
        final TokenCache tokenCache = cache.get(userId, TokenCache.class);
        ...
        if (tokenCache.getExpired().getTime() - Instant.now().toEpochMilli() < 0) {
            throw Code.TIMEOUT.exception("token 过期");
        }
