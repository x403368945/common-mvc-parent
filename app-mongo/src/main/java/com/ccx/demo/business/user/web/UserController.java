...
public class UserController implements IAuthController<Long, TabUser> {
    ...
    @ApiOperation(value = "1.获取当前登录用户", tags = {"0.0.1"})
    ...
    @ApiOperation(value = "2.分页查询用户表", tags = {"0.0.1"})
    @ApiOperationSupport(
            order = 2,
            ignoreParameters = {"password", "insertTime", "updateTime", "authorities", "avatar", "roleList"}
    )
    @ResponseBody
    @Override
    ...
    @ApiOperation(value = "3.新增用户表", tags = {"0.0.1"})
    @ApiImplicitParam(name = "body", dataType = "TabUser", dataTypeClass = TabUser.class, required = true)
    @ApiOperationSupport(
            order = 3,
            ignoreParameters = {
                    "body.id", "body.uid", "body.deleted", "body.insertTime", "body.insertUserId", "body.insertUserName", "body.updateTime", "body.updateUserId", "body.updateUserName",
                    "body.timestamp", "body.sorts", "body.roles", "body.authorities"
            })
    @ResponseBody
    @Override
    ...
    @ApiOperation(value = "4.修改用户表", tags = {"0.0.1"})
    @ApiImplicitParam(name = "body", dataType = "TabUser", dataTypeClass = TabUser.class, required = true)
    @ApiOperationSupport(
            order = 4,
            ignoreParameters = {
    ...
    @ApiOperation(value = "5.逻辑删除用户表", tags = {"0.0.1"})
    ...
    @PatchMapping
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'UC_delete')")
    @ApiOperation(value = "6.批量逻辑删除用户表", tags = {"0.0.1"})
    ...
    @ApiOperation(value = "7.按 id 和 uid 查询用户表", tags = {"0.0.1"})
    ...
    @ApiOperation(value = "8.修改昵称", tags = {"0.0.1"})
    @ApiOperationSupport(
            order = 8,
