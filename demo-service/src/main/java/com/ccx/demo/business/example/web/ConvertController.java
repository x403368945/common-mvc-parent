...
import com.ccx.demo.business.example.entity.TabConvert.OrderBy;
    ...
    public Result<?> save(@AuthenticationPrincipal final TabUser user,
        ...
        return new Result<TabConvert>(1) // 指定接口最新版本号
                .version(this.getClass(), builder -> builder
                        .props(TabConvert.Props.list()) // 当前返回对象属性说明
                        ...
                        .demo(v -> v.setDemo(
                                ...
                                TabConvert.builder() // 当前接口参考案例请求参数，一般demo中存放必填字段或者所有字段
                                        .build()
                        ))
                )
                .execute(result -> result
                        ...
                        .setSuccess(service.save(
                                Param.of(param).required().parseObject(TabConvert.class),
                                user.getId()
                        ))
                );
    }
        ...
        return new Result<>(1) // 指定接口最新版本号
                .version(this.getClass(), builder -> builder
                        ...
                        .demo(v -> v.setDemo(URL.SERVER.append(v.formatUrl()), // 当前接口参考案例请求地址；
                                ...
                                Arrays.asList(
                                        TabConvert.builder().id(100L).uid(Util.uuid32()).build(),
                                        TabConvert.builder().id(101L).uid(Util.uuid32()).build()
                                )
                        ))
                )
                .execute(result -> result
                        ...
                        .call(() -> service.markDelete(Param.of(param).required().hasArray().parseArray(TabConvert.class), user.getId())) // 方案2：按 ID 和 UUID 逻辑删除
                );
    }
    ...
    public Result<?> search(
        ...
        return new Result<TabConvert>(1) // 指定接口最新版本号
                .version(this.getClass(), builder -> builder
                        ...
                        .build()
                        .demo(v -> v.setDemo(URL.SERVER.append(v.formatUrl()), // 当前接口参考案例请求地址；
                                TabConvert.builder() // 当前接口参考案例请求参数，demo中设置支持查询的字段
                                    .deleted(Radio.NO)
                ...
                .execute(result -> result
                        ...
                        .setSuccess(service.findList(
                                Param.of(json).parseObject(TabConvert.class)
                        ))
                );
    }
