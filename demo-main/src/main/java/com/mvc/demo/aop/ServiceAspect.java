...
public class ServiceAspect implements IServiceAspect {
    ...
    @Before("execution(* com.mvc..*.service..*.save(..))")
    public void beforeSave(final JoinPoint joinPoint) {
        save(joinPoint);
    }

    @Before("execution(* com.mvc..*.service..*.saveAll(..))")
    public void beforeSaveAll(final JoinPoint joinPoint) {
        saveAll(joinPoint);
    }

    @Before("execution(* com.mvc..*.service..*.update(..))")
    public void beforeUpdate(final JoinPoint joinPoint) {
        update(joinPoint);
    }

    @AfterReturning(pointcut = "execution(* com.mvc..*.service..*.delete*(..))", returning = "result")
